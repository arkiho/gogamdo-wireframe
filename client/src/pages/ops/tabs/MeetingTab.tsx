      ...
      <CardHeader className="flex-row items-center justify-between">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button size="sm"><Plus className="w-4 h-4 mr-1" />회의록 작성</Button>
          </DialogTrigger>
          <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-3 mt-2">
              ...
              <div>
                <Label>제목 *</Label>
                <Input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="회의 제목" />
              </div>
              <div>
                <Label>참석자</Label>
                <Input value={form.attendees} onChange={e => setForm(f => ({ ...f, attendees: e.target.value }))} placeholder="홍길동, 김철수, 이영희" />
              </div>
              <div>
                <Label>안건 *</Label>
              ...
              <Button onClick={handleCreate} className="w-full" disabled={createMeeting.isPending}>
      ...
      <CardContent>
        {meetings.isLoading ? (
          ...
          <div className="space-y-3">
            {meetings.data.map(m => (
              <div key={m.id} className="p-4 border rounded-lg hover:bg-accent/30 transition-colors">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <h4 className="font-semibold">{m.title}</h4>
                    <Badge variant="outline" className="text-xs">{TYPE_LABELS[m.meetingType] ?? m.meetingType}</Badge>
                  </div>
                ...
                <div className="text-sm space-y-2">
                  <div>
                    <span className="font-medium text-xs text-muted-foreground">안건</span>
                    <p className="whitespace-pre-wrap">{m.agenda}</p>
                  </div>
                  {m.decisions && (
                    <div className="p-2 bg-green-50 rounded">
                      <span className="font-medium text-xs text-green-700">결정사항</span>
                      <p className="text-green-800 whitespace-pre-wrap">{m.decisions}</p>
                    </div>
                  )}
                  {m.actionItems && (
                    <div className="p-2 bg-blue-50 rounded">
                      <span className="font-medium text-xs text-blue-700">후속 조치</span>
