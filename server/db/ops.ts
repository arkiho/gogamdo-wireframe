...
export async function getClientDashboardStats() {
  const db = await getDb();
  if (!db) return { totalClients: 0, activeClients: 0, pendingClients: 0, recentClients: [] as any[] };

  // CRM 고객 통계
  const { crmClients } = await import("../../drizzle/schema");
  const [totalRow] = await db.select({ count: sql<number>`count(*)` }).from(crmClients);
  
  // 최근 고객 5명
  const recentClients = await db.select({
    id: crmClients.id,
    companyName: crmClients.companyName,
  ...
  }).from(crmClients).orderBy(desc(crmClients.createdAt)).limit(5);

  // CRM 딜 통계
  const { crmDeals } = await import("../../drizzle/schema");
  const [activeDeals] = await db.select({ count: sql<number>`count(*)` }).from(crmDeals)
    .where(sql`${crmDeals.stage} NOT IN ('won', 'lost')`);
  const [wonDeals] = await db.select({ count: sql<number>`count(*)` }).from(crmDeals)
    .where(eq(crmDeals.stage, "won"));

  // 고객 포털 고객 통계
  const { clients } = await import("../../drizzle/schema");
  const [portalTotal] = await db.select({ count: sql<number>`count(*)` }).from(clients);
  const [portalActive] = await db.select({ count: sql<number>`count(*)` }).from(clients)
    .where(eq(clients.status, "active"));
  const [portalPending] = await db.select({ count: sql<number>`count(*)` }).from(clients)
  ...
  return {
    ...
    portalTotal: portalTotal?.count ?? 0,
    portalActive: portalActive?.count ?? 0,
    portalPending: portalPending?.count ?? 0,
    recentClients,
  };
}
...
export async function getStaffDashboardStats() {
  const db = await getDb();
  if (!db) return { totalStaff: 0, byDepartment: [] as any[], byRole: [] as any[], recentStaff: [] as any[] };

  // 전체 직원 수 (role이 user이면서 department가 none이 아닌 사람 + admin + master)
  ...
  const byDepartment = Object.entries(deptMap).map(([dept, count]) => ({
    department: dept,
    label: deptLabels[dept] || dept,
    count,
  }));
  ...
  return {
    ...
    byDepartment,
    byRole,
    recentStaff: allUsers.slice(0, 5),
  };
}
...
export async function getPartnerDashboardStats() {
  const db = await getDb();
  if (!db) return { totalPartners: 0, activePartners: 0, pendingRegistrations: 0, activeContracts: 0, recentPartners: [] as any[], pendingRegs: [] as any[] };

  // 등록된 협력사 수
  const [totalRow] = await db.select({ count: sql<number>`count(*)` }).from(opsSubcontractors);
  const [activeRow] = await db.select({ count: sql<number>`count(*)` }).from(opsSubcontractors)
    .where(eq(opsSubcontractors.isActive, 1));

  // 대기 중 등록 요청
  const [pendingRegRow] = await db.select({ count: sql<number>`count(*)` }).from(subRegistrationRequests)
    .where(sql`${subRegistrationRequests.status} IN ('pending', 'staff_approved')`);

  // 활성 계약 수
  const [activeContractRow] = await db.select({ count: sql<number>`count(*)` }).from(subContracts)
    .where(eq(subContracts.status, "active"));

  // 만료 임박 계약 (30일 이내)
  const [expiringRow] = await db.select({ count: sql<number>`count(*)` }).from(subContracts)
    .where(and(
      eq(subContracts.status, "active"),
      sql`${subContracts.endDate} <= DATE_ADD(NOW(), INTERVAL 30 DAY)`,
      sql`${subContracts.endDate} >= NOW()`
    ));

  // 발주서 통계
  const [totalPO] = await db.select({ count: sql<number>`count(*)` }).from(purchaseOrders);
  const [pendingPO] = await db.select({ count: sql<number>`count(*)` }).from(purchaseOrders)
    .where(eq(purchaseOrders.status, "draft"));

  // 최근 등록 협력사 5개
  const recentPartners = await db.select({
    id: opsSubcontractors.id,
    companyName: opsSubcontractors.companyName,
    specialty: opsSubcontractors.specialty,
    contactName: opsSubcontractors.contactName,
    rating: opsSubcontractors.rating,
    isActive: opsSubcontractors.isActive,
    createdAt: opsSubcontractors.createdAt,
  }).from(opsSubcontractors).orderBy(desc(opsSubcontractors.createdAt)).limit(5);

  // 대기 중 등록 요청 목록
  const pendingRegs = await db.select({
    id: subRegistrationRequests.id,
    companyName: subRegistrationRequests.companyName,
    contactName: subRegistrationRequests.contactName,
    status: subRegistrationRequests.status,
    createdAt: subRegistrationRequests.createdAt,
  }).from(subRegistrationRequests)
    .where(sql`${subRegistrationRequests.status} IN ('pending', 'staff_approved')`)
    .orderBy(desc(subRegistrationRequests.createdAt))
    .limit(5);

  return {
    totalPartners: totalRow?.count ?? 0,
    activePartners: activeRow?.count ?? 0,
    pendingRegistrations: pendingRegRow?.count ?? 0,
    activeContracts: activeContractRow?.count ?? 0,
    expiringContracts: expiringRow?.count ?? 0,
    totalPurchaseOrders: totalPO?.count ?? 0,
    pendingPurchaseOrders: pendingPO?.count ?? 0,
    recentPartners,
    pendingRegs,
  };
}
