...
- [ ] 차트: Chart.js 라이브러리 설치 및 설정
- [ ] 차트: 백엔드 통계 API 확장 (월별 지출 추이, 프로젝트별 원가 집행률, 공정 진행률)
- [ ] 차트: OpsHome에 통계 차트 섹션 추가 (월별 지출 추이 라인차트, 프로젝트 상태 분포 도넛차트)
- [ ] 차트: 프로젝트 상세에 원가 집행률 바차트 + 공정 진행률 차트 추가

## OpsX 기능 확장 5: 하도급 업체 평가 시스템
- [ ] 평가: DB 스키마 설계 (opsSubEvaluations 테이블 - 품질/일정/협업도/안전/종합 점수)
- [ ] 평가: DB 마이그레이션
- [ ] 평가: 백엔드 API (평가 CRUD + 업체별 평균 점수 조회)
- [ ] 평가: 프론트엔드 평가 폼 (프로젝트 완료 시 업체별 평가 작성)
- [ ] 평가: 업체 상세에 평가 이력 및 평균 점수 표시
- [ ] 평가: 하도급 관리 탭에 업체 평점 표시
