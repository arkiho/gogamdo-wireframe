...
## 고객 포털 A그룹: 즉시 구현 가능 기능 (2026-03-21)
- [x] A1: 프로젝트 진행 상태 타임라인 (시각적 프로그레스 바 + 단계별 완료 표시)
- [x] A2: 실시간 알림 시스템 (프로젝트 상태 변경 시 이메일/포털 내 알림 + ClientNotificationBell 컴포넌트)
...
## 포트폴리오 드래그 앤 드롭 순서 변경 (2026-06-15)
- [x] DB 스키마에 sortOrder 필드 추가 및 마이그레이션
- [x] 백엔드 순서 변경 API (reorder mutation) 구현
- [x] 프론트엔드 드래그 앤 드롭 UI 구현 (관리자 포트폴리오 페이지)
- [x] Home.tsx FEATURED_PROJECTS 실제 6개 프로젝트로 업데이트

## 포트폴리오 관리 페이지 버그 수정 (2026-06-30)
- [x] 포트폴리오 수정 화면에서 업로드된 사진이 보이지 않는 버그 수정
- [x] 홈페이지에 있는 프로젝트들이 포트폴리오 관리 페이지에 표시되지 않는 버그 수정
- [x] 새 포트폴리오 생성 버튼이 작동하지 않는 버그 수정

## 포트폴리오 이미지 삭제 버그 (2026-07-06)
- [x] 포트폴리오 수정 시 업로드된 이미지가 삭제되지 않는 버그 수정
...
## 포트폴리오 정적 데이터 DB 이관 (2026-07-07)
- [x] images.ts의 정적 프로젝트 데이터(허시드, 제조시설 리모델링 등)를 DB로 이관
- [x] DB에 이미 존재하는 프로젝트(LAB543, 페이퍼랩, 휴레이, SBA/SETEC)는 정적 데이터의 상세 정보로 업데이트
